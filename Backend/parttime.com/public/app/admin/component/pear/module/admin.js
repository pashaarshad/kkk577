layui.define(['message', 'table', 'jquery', 'element', 'yaml', 'form', 'tab', 'menu', 'frame', 'theme', 'convert','fullscreen'],
	function(exports) {
		...
		var pearAdmin = new function() {
			...
			this.bodyRender = function(param) {
				...
				if (isMuiltTab(param) === "true" || isMuiltTab(param) === true) {
					window.bodyTab = bodyTab = pearTab.render({
						elem: 'content',
						roll: true,
						tool: true,
						width: '100%',
			...
			this.themeRender = function(option) {
				...
				if (control === null) {
					...
					if (option.theme.allowCustom === false) {
						control = true;//option.menu.control;
					}
				}
		...
		body.on("click", ".setting", function() {
			...
			var moreItem =
				'<div class="layui-form-item"><div class="layui-input-inline"><input type="checkbox" name="control" lay-filter="control" disabled lay-skin="switch" lay-text="开|关" value="true"></div><span class="set-text">菜单</span></div>';
		...
		function isControl(option) {
			if (option.theme.allowCustom) {
				if (localStorage.getItem("control") != null) {
					...
					return true;//option.menu.control
				}
			} else {
