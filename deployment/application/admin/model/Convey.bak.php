...
class Convey extends Model
{
    ...
    public function get_user_group_rule($uid, $group_id)
    {
        ...
        $lastOrder = Db::name('xy_convey')
            ...
            ->order('oid desc')
            ->find();
        if (!empty($lastOrder) && !$lastOrder['clear_group'] && $lastOrder['group_rule_num'] <= $groupInfo['order_num']) {
            // *teemo*
            // echo 'aa';
            ...
            // echo 'b';
            $lastOrder_old = Db::name('xy_convey')
                ->where('uid', $uid)
                // ->where('group_id', $group_id)
                ->order('oid desc')
                ->find();
            if (!empty($lastOrder_old) && !$lastOrder_old['clear_group'] && $lastOrder['group_rule_num'] < $groupInfo['order_num']) {
                $orderNum_Full = $lastOrder_old['group_rule_num'];
            }
        }
